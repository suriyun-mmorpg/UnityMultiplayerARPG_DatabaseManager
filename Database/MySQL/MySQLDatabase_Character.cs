...
namespace MultiplayerARPG.MMO
{
    public partial class MySQLDatabase
    {
        private void FillCharacterAttributes(IPlayerCharacterData characterData)
        {
            ...
            {
                _logger.LogCritical(ex, "Transaction, Error occurs while replacing attributes of character: " + characterData.Id);
        ...
        private void FillCharacterBuffs(IPlayerCharacterData characterData)
        {
            ...
            {
                _logger.LogCritical(ex, "Transaction, Error occurs while replacing buffs of character: " + characterData.Id);
                transaction.Rollback();
            }
            transaction.Dispose();
            connection.Close();
        ...
        private void FillCharacterHotkeys(IPlayerCharacterData characterData)
        {
            ...
            {
                _logger.LogCritical(ex, "Transaction, Error occurs while replacing hotkeys of character: " + characterData.Id);
                transaction.Rollback();
            }
            transaction.Dispose();
            connection.Close();
        ...
        private void FillCharacterItems(IPlayerCharacterData characterData)
        {
            ...
            {
                _logger.LogCritical(ex, "Transaction, Error occurs while replacing items of character: " + characterData.Id);
                transaction.Rollback();
            }
            transaction.Dispose();
            connection.Close();
        ...
        private void FillCharacterQuests(IPlayerCharacterData characterData)
        {
            ...
            {
                _logger.LogCritical(ex, "Transaction, Error occurs while replacing quests of character: " + characterData.Id);
                transaction.Rollback();
            }
            transaction.Dispose();
            connection.Close();
        ...
        private void FillCharacterCurrencies(IPlayerCharacterData characterData)
        {
            ...
            {
                _logger.LogCritical(ex, "Transaction, Error occurs while replacing currencies of character: " + characterData.Id);
                transaction.Rollback();
            }
            transaction.Dispose();
            connection.Close();
        ...
        private void FillCharacterSkills(IPlayerCharacterData characterData)
        {
            ...
            {
                _logger.LogCritical(ex, "Transaction, Error occurs while replacing skills of character: " + characterData.Id);
                transaction.Rollback();
            }
            transaction.Dispose();
            connection.Close();
        ...
        private void FillCharacterSkillUsages(IPlayerCharacterData characterData)
        {
            ...
            {
                _logger.LogCritical(ex, "Transaction, Error occurs while replacing skill usages of character: " + characterData.Id);
                transaction.Rollback();
            }
            transaction.Dispose();
            connection.Close();
        ...
        public override void DeleteCharacter(string userId, string id)
        {
            ...
            if (count > 0)
            {
                ...
                {
                    _logger.LogCritical(ex, "Transaction, Error occurs while deleting character: " + id);
                    transaction.Rollback();
                }
                transaction.Dispose();
                connection.Close();
